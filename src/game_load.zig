...
fn maybeGetBallBoxCollision(ball_pos: math.Vec2f, radius: f32, box_pos: math.Vec2f, box_half_extents: math.Vec2f) ?BoxCollision {
    ...
    const clamped: math.Vec2f = [_]f32{
    ...
    const collis_pos = box_pos + clamped;
    const difference = collis_pos - ball_pos;
    const distance = math.magVec2f(difference);
    const collis_dir = math.normVec2f(difference);
    if (distance < radius) {
        //std.debug.warn("[INFO] diff: {} clamped: {} difference: {} distance: {}\n", .{ center_rela_vec, clamped, difference, distance });
...
pub fn maybeGetBallBrickCollision(ball_pos: math.Vec2f, radius: f32, game_level: GameLevel) ?BrickCollision {
    ...
    const brick_collision: ?BrickCollision = blk: for (row_list.items) |r| {
        var c: usize = 0;
...
pub fn createLevel(allocator: *Allocator, brick_size_x: f32, brick_size_y: f32, tiles: []const []const u8) !GameLevel {
